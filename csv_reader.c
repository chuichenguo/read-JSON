...
void readMaliciousInformation(char *file_name, char **output_string)
{
    ...
    char malicious_file[1000] = {0};
    char line[100] = {0};
    size_t len = 0, tot_len = 0;

    while (fgets(line, 20, (FILE *)fp) != NULL)
    {
        strcat(malicious_file, line);
        tot_len += strlen(line);
    }
    // printf("%s", malicious_file);

    cJSON *cjson_malicious_file = NULL;
    cJSON *cjson_ip = NULL;
    int ip_array_size = 0;
    cJSON *cjson_ip_content = NULL;
    char malicious_ip_address[100] = {0};
    cjson_malicious_file = cJSON_Parse(malicious_file);
    if (cjson_malicious_file == NULL)
    {
        printf("parse fail.\n");
        // return -1;
    }
    cjson_ip = cJSON_GetObjectItem(cjson_malicious_file, "IP");
    ip_array_size = cJSON_GetArraySize(cjson_ip);
    for (int i = 0; i < ip_array_size; i++)
    {
        cjson_ip_content = cJSON_GetArrayItem(cjson_ip, i);
        strcat(malicious_ip_address, cjson_ip_content->valuestring);
        if (i != ip_array_size - 1)
        {
            strcat(malicious_ip_address, ",");
        }
    }
    printf("%s", malicious_ip_address);
    *output_string = (char *)calloc(tot_len + 1, sizeof(char *));
    strcpy(*output_string, malicious_ip_address);
    fclose(fp);
}
